<script setup>
import { MapPin, Edit2, Trash2, XCircle, AlertOctagon, CheckSquare, Shirt } from 'lucide-vue-next';
         ...
         <div v-for="match in matches" :key="match.id" 
            ...
            <div class="flex items-start sm:items-center gap-4 flex-1 min-w-0">
              ...
              <div class="min-w-0 flex-1">
                 <div class="flex items-center gap-2 mb-0.5">
                   ...
                   <!-- Kit Badge -->
                   <span v-if="match.kit" class="text-[10px] flex items-center gap-1 px-1.5 py-0.5 rounded border"
                         :class="match.kit === 'Home' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-200'">
                 ...
                 <div class="font-bold text-slate-900 text-lg truncate flex items-center gap-2 flex-wrap">
                    ...
                    <span v-if="match.status !== 'Scheduled'" class="text-[10px] px-1.5 py-0.5 rounded text-white uppercase tracking-wide"
                          :class="{
                            'bg-emerald-500': match.status === 'Played',
                            'bg-rose-500': match.status === 'Cancelled',
                            'bg-amber-500': match.status === 'Postponed'
                          }">
                      {{ match.status }}
                    </span>
                 </div>
            ...
            <div class="flex items-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity self-end sm:self-auto">
               ...
               <div v-if="match.status === 'Scheduled'" class="flex gap-1">
                   <button @click="$emit('updateStatus', match, 'Played')" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Played">
               ...
               <button v-else @click="$emit('updateStatus', match, 'Scheduled')" class="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg text-xs font-bold">
                 Restore
               </button>
